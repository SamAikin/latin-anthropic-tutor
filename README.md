...
**Grading and evaluation are the exception**: grading homework and evaluating reading translations should be done with the bigger (Opus-class) model, like planning.
A confidently wrong correction teaches the error, and as the learner I cannot be relied upon to notice a subtly wrong diagnosis — so the asymmetry runs the other way here: executing a lesson plan is cheap, but judging my Latin is not.
